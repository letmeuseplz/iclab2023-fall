...
module PATTERN (
    ...
    task task_read_pattern; begin
        ...
        for (i=0; i<IMG_LEN; i=i+1) scan_val = $fscanf(f_img, "%h", pat_img[i]);
        for (i=0; i<KER_LEN; i=i+1) scan_val = $fscanf(f_ker, "%h", pat_kernel[i]);
        for (i=0; i<WGT_LEN; i=i+1) scan_val = $fscanf(f_wgt, "%h", pat_weight[i]);
        
        scan_val = $fscanf(f_out, "%h", golden_ans);
    end endtask
