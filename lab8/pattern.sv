...
module PATTERN (
    ...
    reg [8*200:1] line_buffer; // 暫存整行字串的 buffer，用來安全過濾掉註解
    ...
    task task_read_pattern; begin
        // 💡 [終極修改] 讀取 Option：先用 fgets 抓整行，再用 sscanf 抓數字，完美過濾掉 "// Option: X"
        scan_val = $fgets(line_buffer, f_opt);
        if (scan_val == 0) begin
            $display("\033[1;33m[INFO] End of file reached.\033[0m");
            task_pass_message();
            $finish;
        end
        scan_val = $sscanf(line_buffer, "%d", pat_opt);
